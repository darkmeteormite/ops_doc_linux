...
1、完全备份
	...
	* --no-timestamp选项：此选项指示innobackupex不要创建带有时间戳的目录来存储备份
	# innobackupex --user=root /backup/mysql/`date +%F` --no-timestamp
	# innobackupex --user=root --incremental /data/backup/mysql/0309 --incremental-basedir=/data/backup/mysql/2017-03-09/ --no-timestamp
	*--incremental表示增量备份的目录，--incremental-basedir表示上一次备份的目录；
	...
	(2)xtrabackupbinloginfo —— mysql服务器当前正在使用的二进制日志文件及至备份这一刻为止二进制日志事件的位置。
	(3)xtrabackupbinlogpos_innodb —— 二进制日志文件及用于InnoDB或XtraDB表的二进制日志文件的当前position。
...
2、准备(prepare)一个完全备份
	...
	* --use-memory=4G
	准备过程可以通过使用更多的内存加快。它取决于系统上可用RAM，它默认为100MB。一般来说，进程可用的内存越多越好。进程中使用的内存量可以通过字节数的倍数指定
...
4、使用innobackupex进行增量备份
	...
	其中BASE-DIR指的是完全备份所在的目录，而INCREMENTAL-DIR-1指的是第一次增量备份的目录，INCREMENTAL-DIR-2指的是第二次增量备份的目录，其它依次类推，即如果有多次增量备份，每一次都要执行如上操作； 注意：这里增量备份所在的目录只识别绝对路径； 每次合并增量备份可在完整备份目录里面的xtrabackup_checkpoints查看到合并进度
	...
	#将基准与所有增量合并后，可以准备它来回滚未提交的事务
	innobackupex --apply-log /backup/fullbackup/2015-04-15_16-30-19/ 
...
5、Xtrabackup的“流”及“备份压缩”功能
	...
	此外，在执行本地备份时，还可以使用–parallel选项对多个文件进行并行复制。此选项用于指定在复制时启动的线程数目。当然，在实际进行备份时要利用此功能的便利性，也需要启用innodbfilepertable选项或共享的表空间通过innodbdatafilepath选项存储在多个ibdata文件中。对某一数据库的多个文件的复制无法利用到此功能。其简单使用方法如下：
...
6、导入或导出单张表
	...
	默认情况下，InnoDB表不能通过直接复制表文件的方式在mysql服务器之间进行移植，即便使用了innodbfilepertable选项。而使用Xtrabackup工具可以实现此种功能，不过，此时需要“导出”表的mysql服务器启用了innodbfilepertable选项（严格来说，是要“导出”的表在其创建之前，mysql服务器就启用了innodbfilepertable选项），并且“导入”表的服务器同时启用了innodbfilepertable和innodbexpandimport选项。
...
7、使用Xtrabackup对数据库进行部分备份
	...
	Xtrabackup也可以实现部分备份，即只备份某个或某些指定的数据库或某数据库中的某个或某些表。但要使用此功能，必须启用innodbfileper_table选项，即每张表保存为一个独立的文件。同时，其也不支持–stream选项，即不支持将数据通过管道传输给其它程序进行处理。
